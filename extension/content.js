if (window.location.hostname.includes('xnhau')) {
  ...
  if (window !== window.top) {
    ...
    // Chuyển tiếp sự kiện click cho ứng dụng cha để xử lý Play/Pause và Double Click
    let clickTimeout = null;
    document.addEventListener("click", (e) => {
      // Đừng chặn click vào Captcha hoặc các nút skip ad
      if (e.target.closest('#cf-wrapper, .cf-turnstile, .skip-button, .vjs-ad-skip-button')) return;
      ...
      if (clickTimeout !== null) {
        ...
        clickTimeout = null;
        window.parent.postMessage({ type: "XTOK_DOUBLE_CLICK" }, "*");
      } else {
        // Single click
        clickTimeout = setTimeout(() => {
          clickTimeout = null;
          window.parent.postMessage({ type: "XTOK_CLICK" }, "*");
        }, 250);
      }
    }, true);
  }
