...
const VideoCard = ({ video }) => {
  ...
  useEffect(() => {
    const handleVideoState = (event) => {
      ...
      if (event.data.type === "XTOK_VIDEO_STATE") {
        setCurrentTime(event.data.currentTime || 0);
        setDuration(event.data.duration || 0);
        setPlaying(!event.data.paused);
      } else if (event.data.type === "XTOK_CLICK") {
        // Toggle play from iframe click
        if (iframeRef.current && iframeRef.current.contentWindow) {
          const newState = !playing;
          iframeRef.current.contentWindow.postMessage({ type: newState ? "XTOK_PLAY" : "XTOK_PAUSE" }, "*");
          setPlaying(newState);
        }
      } else if (event.data.type === "XTOK_DOUBLE_CLICK") {
        // Create heart animation at center
        const clickOverlay = document.querySelector('.click-overlay');
        if (clickOverlay) {
          // Simulate double click to trigger heart
          const ev = new MouseEvent('dblclick', {
            bubbles: true, cancelable: true, clientX: window.innerWidth / 2, clientY: window.innerHeight / 2
          });
          clickOverlay.dispatchEvent(ev);
        }
      }
    };
